...
class Command
{
  public:
	...
	bool CheckNickNameDuplicate(std::string					name,
								std::map<int, class User*>& userList);
	...
	void		ChannelPART(int fd, std::string cannelName,
							std::vector<std::string> commandVec);
	void		MsgToAllChannel(int target, std::string channelName,
								std::string command, std::string msg);
	void		NickMsgToAllChannel(int target, std::string channelName,
									std::string command, std::string msg);
