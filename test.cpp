...
int main(int ac, char **av)
{
	...
	server_addr.sin_port =
		htons(6667);  // 호스트 바이트 순서 -> 네트워크 바이트 순서
	...
	while (1)
	{
		...
		if (strncmp(readbuffer, "CAP REQ", 7) == 0)
		{
			std::string reqResponse =
				":server_name CAP * ACK :multi-prefix\r\n";
			send(client_fd, reqResponse.c_str(), reqResponse.size(), 0);
		}

		if (strncmp(readbuffer, "NICK", 4) == 0)
		{
			std::string nickResponse =
				":server_name 001 Welcome to the IRC server\r\n";
		...
		{
			std::string response =
				":server_name 001 zinox :Welcome to the IRC server\r\n";
			send(client_fd, response.c_str(), response.size(), 0);
			response =
				":server_name 002 zinox :Your host is server_name, running "
				"version 1.0\r\n";
			send(client_fd, response.c_str(), response.size(), 0);
			// MOTD 전송
			response =
				":server_name 375 zinox :- server_name Message of the Day "
				"-\r\n";
			send(client_fd, response.c_str(), response.size(), 0);
...
// 1, 서버의 fd, 2. 클라이언트의 주정보 즉 서버 addr과 같음. 3.클라이언트
// 구조체의 길이.
